    ...
    <div className="min-h-screen bg-background">
      ...
      <article className="pt-28 pb-20">
        <div className="container mx-auto px-4 max-w-3xl">
          ...
          <img src={post.image} alt={post.title} width={800} height={512} className="w-full h-64 md:h-96 object-cover rounded-lg mb-8" />
          ...
          <h1 className="text-3xl md:text-4xl lg:text-5xl font-heading font-bold text-foreground mb-8 leading-tight">{post.title}</h1>
          ...
          <div className="mt-12 pt-8 border-t border-border">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {blogPosts.filter((p) => p.slug !== slug).slice(0, 4).map((related) => (
                <Link key={related.slug} to={`/blog/${related.slug}`} className="group flex gap-4 items-start p-3 rounded-lg hover:bg-card transition-colors">
                  <img src={related.image} alt={related.title} loading="lazy" width={80} height={80} className="w-20 h-20 object-cover rounded shrink-0" />
                  <h4 className="font-heading text-sm font-semibold text-foreground group-hover:text-primary transition-colors leading-snug">{related.title}</h4>
                </Link>
              ))}
            </div>
          </div>
