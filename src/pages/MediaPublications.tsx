    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="pt-28 pb-20 bg-background">
        <div className="container mx-auto px-4 max-w-5xl">
          <ScrollReveal>
            <h1 className="text-4xl md:text-5xl font-heading font-bold text-foreground mb-4 text-center">{t.mediaPage.title}</h1>
            <p className="text-muted-foreground text-center mb-12 max-w-xl mx-auto">{t.mediaPage.subtitle}</p>
          </ScrollReveal>
                    ...
                    <div className={`grid ${pub.images.length === 1 ? 'grid-cols-1' : pub.images.length === 2 ? 'grid-cols-2' : 'grid-cols-2 md:grid-cols-3'} gap-1`}>
                      {pub.images.map((img, imgIdx) => (
                        <div key={imgIdx} className="overflow-hidden cursor-pointer" onClick={() => openLightbox(img)}>
                          <img src={img} alt={`${pub.title} - image ${imgIdx + 1}`} loading="lazy" className="w-full h-48 md:h-56 object-cover hover:scale-105 transition-transform duration-500" />
                        </div>
                      ))}
                    </div>
                  )}
                  <div className="p-6">
                    <span className="text-xs font-heading font-semibold tracking-wider text-primary uppercase">{pub.source}</span>
                    <h3 className="font-heading text-lg font-bold text-foreground mt-1 mb-2">{pub.title}</h3>
                    <p className="text-muted-foreground text-sm leading-relaxed">{pub.description}</p>
                    {"audioUrl" in pub && pub.audioUrl && (
                      <a href={pub.audioUrl} target="_blank" rel="noopener noreferrer" className="inline-block mt-3 text-primary hover:brightness-110 font-heading text-sm font-semibold tracking-wider transition-all">
